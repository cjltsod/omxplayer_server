...
def omx_panel(request):
    available_command = [
        ...
        {'cmd': 'reboot', 'description': 'Restart RPi', 'icon': 'fa-power-off'},
        {'cmd': 'update', 'description': 'Update software', 'icon': 'fa-download'},
    ...
    try:
        ...
        file_list = os.listdir(target_dir)
        onlydirs = [ f for f in file_list if f[0] != '.' and os.path.isdir(os.path.join(target_dir ,f))]
